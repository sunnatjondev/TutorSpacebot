                    ...
                    {visibleStudents.map((student, index) => (
                      <div key={student.id}>
                        <div className="flex items-center gap-3 py-3">
                          ...
                          <button
                            onClick={() => {
                              const inviteUrl = `https://t.me/${botUsername}?start=parent_${student.id}`
                              navigator.clipboard.writeText(inviteUrl)
                              haptic?.success?.()
                              if (window.Telegram?.WebApp?.showAlert) {
                                window.Telegram.WebApp.showAlert(lang === 'ru' ? 'Ссылка для родителей скопирована!' : 'Ota-ona uchun taklif havolasi nusxalandi!')
                              } else {
                                alert(lang === 'ru' ? 'Ссылка для родителей скопирована!' : 'Ota-ona uchun taklif havolasi nusxalandi!')
                              }
                            }}
