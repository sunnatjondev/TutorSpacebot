...
export default function TeacherSchedule() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface-lowest">
                        ...
                        {!isDone && (
                          <button
                            ...
                            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full active:scale-90 transition-transform disabled:opacity-50 ${
                              isInProgress ? 'bg-paid-green text-white' : 'bg-primary text-on-primary'
                            }`}
      ...
      <button
        className="fixed bottom-[96px] right-4 h-16 w-16 flex items-center justify-center rounded-[24px] bg-primary text-on-primary shadow-m3-elevation-3 transition-all duration-300 active:scale-90 z-40"
        onClick={() => {
          haptic?.medium()
          setShowCreate(true)
        }}
