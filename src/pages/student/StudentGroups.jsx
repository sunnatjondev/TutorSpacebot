  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface-lowest">
      ...
      <div className="page-wrapper px-4 pt-5">
        <h1 className="text-[28px] font-extrabold text-on-surface mb-5">
          {t('studentGroups.title')}
        </h1>

        <div className="space-y-4">
          {mockStudentGroups.map((group, i) => (
            <div
              key={group.id}
            ...
            >
              {/* Header */}
              <h2 className="text-xl font-extrabold text-on-surface mb-1">{group.subject}</h2>
              ...
              <div className="rounded-card p-4 mb-4" style={{ background: '#2a2933' }}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="font-bold text-on-surface">{group.teacher}</p>
                    <span className="chip text-[11px]">{group.teacherSubject}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                  ...
                  <div className="rounded-xl bg-surface-container p-3">
                    ...
                    <p className="flex items-center gap-1.5 font-bold text-on-surface text-sm">
                      <CalendarDays size={14} className="text-primary" /> {group.nextLesson}
                    </p>
                  </div>
                </div>
              </div>
                ...
                {group.members.map((m, mi) => (
                  <div key={mi}>
                    <div className="flex items-center gap-3 py-2.5">
                      ...
                      <p className="flex-1 text-on-surface text-sm font-medium">{m.name}</p>
                      {m.role === 'teacher' && (
                        <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-brand/20 border border-brand/30 text-primary text-xs font-bold">
